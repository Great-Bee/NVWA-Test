 ...
 */
public class CaseU00002003 extends BaseTestCase {
...
//    private static final Logger logger = Logger.getLogger(CaseU00002001.class);
    ...
    @Test
    public void testCaseU00002003(){
        logger.info("----------------------test start-------------------------");

        //读取这个JS File 是否存在
        String url = ResponseConstant.HOST + URL_Layout+"/"+ UUID.randomUUID().toString();
        ...
        if (nvwaResponse != null) {
            if (nvwaResponse.isOk()) {
                ...
                //打印message的内容
                logger.info("----------------------test dataMap-------------------------");
                logger.error(JSONObject.fromObject(nvwaResponse).toString());
            }
        } else {
            //返回的response为空
            //test false
            logger.info("[testCaseU00002003][testCaseU00002003]: response is null");
        }
